...
Local nT:= TamSX3("D4_COD")[1] //Tamaño del campo de Producto -->16
...
nTr:= len(alltrim(cProdut))	//Tamaño real
...
if !empty(Posicione("SB1",1,xFilial("SB1")+cProdut,"B1_COD"))
	...
	//If cTipo == .T.
		...
		if (cDat)->(!EOF())
			...
			EndIf
		Else
			cRet:= "No existen Orden de Producción y/o Producto informado, VERIFIQUE!"
		EndIf
		(cDat)->(dbCloseArea())
	//else
